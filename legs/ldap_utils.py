...
def extract_passwords_from_packets(packets):
    ...
    for packet in packets:
        ...
        if packet_has_third_layer:
            ...
            packet_contains_password = contains(
                userPasswordTypeStringAsIntegers, packet_as_integers
            )
            if packet_contains_password:
                to_decode = bytes(packet[3])
                decoded, rest_of_substrate = decode(
                    to_decode, asn1Spec=rfc4511.LDAPMessage()
                )
                decoded_as_dictionary = encode(decoded)

                userpassword_type_to_check = encode(decoded)["protocolOp"][
                    "modifyRequest"
                ]["changes"][0]["modification"]["type"]
                check = []
                for char in userpassword_type_to_check:
                    check.append(char)
                if check == userPasswordTypeStringAsIntegers:
                    found_password_as_bytes = encode(decoded)["protocolOp"][
                        "modifyRequest"
                    ]["changes"][0]["modification"]["vals"][0]
...
def extract_users_uids_from_packets(packets):
    ...
    for packet in packets:
        ...
        if packet_has_third_layer:
            ...
            password_type_exists_at_positions = contains(
                userPasswordTypeStringAsIntegers, packet_as_integers
            )
            uid_type_exists_at_positions = contains(
                userUIDTypeStringAsIntegers, packet_as_integers
            )
            comma_exists_at_positions = contains(
                CommaStringAsIntegers, packet_as_integers
            )

            if (
                uid_type_exists_at_positions is not False
                and password_type_exists_at_positions is not False
            ):
                # We get the text between uid= and ",", take this as reference -> uid=<userUID>,cn=users...
                found_uid = packet_as_integers[
                    uid_type_exists_at_positions[1] : comma_exists_at_positions[0]
                ]
