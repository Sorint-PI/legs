...
def source_ldap_server():
    ldap_server = ldap3.Server(
        "ldap://localhost:4389", use_ssl=False, get_info=ldap3.ALL
    )
    return ldap_server
...
def source_ldap_server_connection(source_ldap_server):
    ...
    ldap_destination_connection = ldap3.Connection(
        source_ldap_server, server_bind_dn, server_bind_password, auto_bind=True
    )
...
def destination_ldap_server():
    ldap_server = ldap3.Server(
        "ldap://localhost:5389", use_ssl=False, get_info=ldap3.ALL
    )
    return ldap_server
...
def destination_ldap_server_connection(destination_ldap_server):
    ...
    ldap_destination_connection = ldap3.Connection(
        destination_ldap_server, server_bind_dn, server_bind_password, auto_bind=True
    )
    return ldap_destination_connection
...
def test_ldap_destination_server_bind():
    ...
    ) = main.establish_ldap_destination_server_connection()
...
def test_ldap_write_password_for_user(
    destination_ldap_server, destination_ldap_server_connection
):
    server_bind_dn = "cn=admin,dc=example,dc=org"
    ...
    main.write_ldap_password_for_user(
        destination_ldap_server_connection, testuser_dn, testpassword
    )
    ...
    main.write_ldap_password_for_user(
        destination_ldap_server_connection, testuser_dn, test_second_password
    )
    # re-bind to test wether the password actually works from LDAP
    conn_second = ldap3.Connection(
        destination_ldap_server, testuser_dn, test_second_password
    )
    if conn_second.bind():
...
def test_async_password_intercepting_and_writing(
    source_ldap_server,
    ...
    destination_server_testuser_dn = (
        "uid=" + testuser + destination_server_users_bind_dn
    )
    ...
    password_intercept_thread = threading.Thread(
    ...
    conn = ldap3.Connection(
        source_ldap_server, source_server_testuser_dn, first_test_password
    )
    if conn.bind():
        first_bind_on_source_successful = True
    # Test the bind on the destination LDAP server
    conn = ldap3.Connection(
        destination_ldap_server, destination_server_testuser_dn, first_test_password
    )
    if conn.bind():
    ...
    # Test the bind on the source LDAP server
    conn = ldap3.Connection(
        source_ldap_server, source_server_testuser_dn, second_test_password
    )
    if conn.bind():
        second_bind_on_source_successful = True
    # Test the bind on the destination LDAP server
    conn = ldap3.Connection(
        destination_ldap_server, destination_server_testuser_dn, second_test_password
    )
    if conn.bind():
...
def test_main(
    ...
    stop_flag = [False]
    intercepting_thread = threading.Thread(
